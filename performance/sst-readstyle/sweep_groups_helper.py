...
def create_sweep_groups(machine_name, writer_np, reader_np_ratio, size_per_pe, marshal, transport, lock,
                        run_repetitions, batch_job_timeout_secs, node_layouts, per_experiment_timeout):
    """
    ...
    marshal:                A List of marshaling types
    transport:              A list of sst data transports
    lock:		    whether or not to lock geometry on both sides
          ...
          for l in lock:
            ...
            for s in size_per_pe:
                for r_ratio in reader_np_ratio:
                    ...
                    config_fname = "manyvars-{}.txt".format(s,r_ratio)
                    scaling = '-w'
                    adios_xml = 'sst.xml'
                    post_hoc = False
